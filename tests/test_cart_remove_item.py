...
def test_remove_item_from_cart_updates_badge(page, login):
    """Verify removing an item updates the cart badge and removes the item from the cart."""
    ...
    cart.remove_item(item_slug)  # remove selected item from cart

    inventory.assert_cart_badge_hidden()  # verify cart badge disappears when cart becomes empty
    cart.assert_item_not_present(item_name)  # confirm removed item is no longer listed in cart
